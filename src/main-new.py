...
class pressureSub():
    ...
    def __init__(self):
        ...
        self.pub_state_change = rospy.Publisher("/umoru_state_change", Int16, queue_size=1)
...
class voiceSub():
    ...
    def __init__(self):
        ...
        self.pub_state_change = rospy.Publisher("/umoru_state_change", String, queue_size=1)
    ...
    def callback(self, data):
        ...
        if INTERACTING_FLAG == True:
            if volume > 70:
                pub_msg_state.data = min(CURRENT_UMORU_STATE + 1, 5)
                pub_msg_state_change.data = "audio"
                self.publish_state(pub_msg_state)
                self.publish_state_change(pub_msg_state_change)
...
class timeController():
    def __init__(self):
        self.timer = rospy.Time(rospy.Duration(1.0), self.timer_callback)
        self.pub = rospy.Publisher("/timer_topic", String, queue_size=10)
        self.pub_state_change = rospy.Publisher("/umoru_state_change", String, queue_size=10)
        
    def timer_callback(self, data):
        msg = String()
        state_change_msg = String()
        msg.data = "This message is published at 1 Hz"
        self.pub.publish(msg)
        if INTERACTING_FLAG == True and len(TIME_CONTROLLER_LIST) != 0 and rospy.get_time() - TIME_CONTROLLER_LIST[-1] > 20:
            state_change_msg.data = "time"
            self.pub_state_change.publish(state_change_msg)
...
class umoruStateController():
    def __init__(self):
        ...
        self.sub_change_status = rospy.Pubslisher("/demo_status")
