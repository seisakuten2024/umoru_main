...
class startAndEndFlag():
    ...
    def __init__(self):
        ...
        self.sub_point_cloud = rospy.Subscriber("/point_cloud_check", Bool, self.callback_point_cloud, queue_size=3)
    ...
    def callback_point_cloud(self, data):
        ...
        pub_msg_demo_status = Bool()
        pub_msg_change_trigger = String()
        face_appear = data.data
        # もし体験中じゃない かつ 前回の体験から10秒以上経過しているとき
        self.history.append(face_appear)
        ratio = np.mean(np.array(self.history))
        rospy.loginfo("ratio = {}".format(ratio))
        if INTERACTING_FLAG == False and (LAST_TIME == None or rospy.get_time() - LAST_TIME >= 10):
            if ratio < 0.2:
                # 顔が見えない場合
                FACE_FIND_FLAG = False
            elif face_appear == True:
                print("-------------------------------- face appeared =================================")
                # 顔が見える場合
                if FACE_FIND_FLAG == True:
                    if rospy.get_time() - self.face_appeared_time >= 1:
                        # もし顔が見えてから3秒経過した場合、体験スタート
                        INTERACTING_FLAG = True
                        print("================= 1. Interaction Start (by camera)  ======================")
                        TIME_CONTROLLER_LIST = []
